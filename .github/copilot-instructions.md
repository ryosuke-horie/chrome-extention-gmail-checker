...
Chrome拡張機能としてGmailの誤送信対策用ツールを作成する。
軽量な素のJavascriptを使用する。
拡張機能の開発用にextention.jsというライブラリを利用している。

## 参考

ブラウザGmailの拡張機能の開発をJQueryでサポートしているgmail.jsの実装を参考にする。
実装時は必ず生のJavascriptに変換すること。
